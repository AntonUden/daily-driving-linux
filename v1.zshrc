...
# ====== Credits ======
# This theme is based on the folowing:
# https://github.com/alejandromume/ubunly-zsh-theme/blob/main/ubunly.zsh-theme
...
if [ "$color_prompt" = yes ]; then
    ...
    # PROMPT=$'%F{%(#.blue.214)}┌──${debian_chroot:+($debian_chroot)──}(%B%F{%(#.red.202)}%n%(#.💀. λ )%m%b%F{%(#.orange.214)})-[%B%F{reset}%(6~.%-1~/…/%4~.%5~)%b%F{%(#.blue.214)}]\n└─%B%(#.%F{red}#.%F{208}$)%b%F{reset} '
    # RPROMPT=$'%(?.. %? %F{red}%B⨯%b%F{reset})%(1j. %j %F{yellow}%B⚙%b%F{reset}.)'
...
preexec() {
    thetimerthattellsushowlongtimeittooktorunthecommand=$(($(date +%s%0N)/1000000))
}

precmd() {
    if [ $thetimerthattellsushowlongtimeittooktorunthecommand ]; then
        now=$(($(date +%s%0N)/1000000))
        elapsed=$(($now-$thetimerthattellsushowlongtimeittooktorunthecommand))
        
        export RPROMPT="%F{cyan}${elapsed}ms %{$reset_color%}"
        unset thetimerthattellsushowlongtimeittooktorunthecommand
    fi
